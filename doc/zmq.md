...
The Bzetcoin Core daemon can be configured to act as a trusted "border
router", implementing the bzetcoin wire protocol and relay, making
...
The ZeroMQ feature in Bzetcoin Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building bzetcoind:
...
For instance:
    ...
    $ bzetcoind -zmqpubhashtx=tcp://127.0.0.1:28332 \
               -zmqpubhashtx=tcp://192.168.1.2:28332 \
               -zmqpubrawtx=ipc:///tmp/bzetcoind.tx.raw \
...
These options can also be provided in bzetcoin.conf.
...
From the perspective of bzetcoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into bzetcoind directly. Furthermore, no information is
...
using. Bzetcoind appends an up-counting sequence number to each

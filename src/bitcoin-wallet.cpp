...
static bool WalletAppInit(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs)) {
        std::string usage = strprintf("%s bzetcoin-wallet version", PACKAGE_NAME) + " " + FormatFullVersion() + "\n\n" +
                                      "bzetcoin-wallet is an offline tool for creating and interacting with " PACKAGE_NAME " wallet files.\n" +
...
int main(int argc, char* argv[])
{
    ...
    if (method.empty()) {
        tfm::format(std::cerr, "No method provided. Run `bzetcoin-wallet -help` for valid methods.\n");
        return EXIT_FAILURE;
    }

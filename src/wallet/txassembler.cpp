    ...
    const bool sign)
{
    ...
    while (true) {
        ...
        // the recipients are funded through the pegout kernel instead of traditional BZET outputs.
        ...
        // Calculate transaction size and the total necessary fee amount (includes BZET and MWEB fees)
    ...
    if (new_tx.mweb_type != MWEB::TxType::BZET_TO_BZET) {
...
bool TxAssembler::AttemptCoinSelection(InProcessTx& new_tx, const CAmount& nTargetValue) const
{
    ...
    if (new_tx.recipients.front().IsMWEB()) {
        ...
        // First try to construct a BZET-to-BZET transaction
        CoinSelectionParams mweb_to_mweb = new_tx.coin_selection_params;
        mweb_to_mweb.input_preference = InputPreference::BZET_ONLY;
        ...
        // If BZET-to-BZET fails, create a peg-out transaction
